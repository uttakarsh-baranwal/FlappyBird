...
function App() {
    ...
    if(startGame) {
      interval = setInterval(() => {
        if(obstacleLeftPosition > -OBSTACLE_WIDTH){
          setObstacleLeftPosition(obstacleLeftPosition => obstacleLeftPosition-6);
        } else {
  ...
  useEffect(() => {
    if(startGame){
      ...
      {
        setStartGame(false);
        setBirdPosition(birdPosition => GAME_HEIGHT - BIRD_SIZE);
      }
    }
  ...
  return (
    <>
      <div 
        ...
        style={{
        overflow: "hidden", //anything that will overflow will be hidden from screen i.e. gamebox
        position: "relative",
